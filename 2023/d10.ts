const input = `F-7JF-F-F|-J.FL|.L-L7.L-L-J.7F-FJ-F7F.F7J-F.J.7.FF7L-7F7|7FFF7FF7FF7.L7J-77FF-7-|-|7.7.J7J7F--J7..FL-F.JFJ.-7F-F7F77-L--F.FF77-7FFLJ-.F|J.F7
...
const openTop = ['J', '|', 'L'];
const openRight = ['L', '-', 'F'];
const openBottom = ['7', '|', 'F'];
const openLeft = ['J', '-', '7'];

const getStart = (lines: string[]): Coordinate => {
    let x = 0;
    let y = 0;
    ...
    return { x, y, type: lines[y][x] }
}

const getCoordinate = (lines: string[], x: number, y: number) => ({
    x, y, type: lines[y].charAt(x)
})

const isValidPipe = (start: Coordinate, pipeToCompare: Coordinate, pipePos: number) => {
    // where pipePos = 0 is top
    // where pipePos = 1 is left
    // where pipePos = 2 is right
    // where pipePos = 3 is bottom
    if (pipePos === 0) {
        return (start.type === 'S' || openTop.includes(start.type)) && openBottom.includes(pipeToCompare.type);
    }
    if (pipePos === 1) {
        return (start.type === 'S' || openLeft.includes(start.type)) && openRight.includes(pipeToCompare.type);
    }
    if (pipePos === 2) {
        return (start.type === 'S' || openRight.includes(start.type)) && openLeft.includes(pipeToCompare.type);
    }
    if (pipePos === 3) {
        return (start.type === 'S' || openBottom.includes(start.type)) && openTop.includes(pipeToCompare.type);
    }
    return false;
}

const getPossibleNodes = (lines: string[], currPos: Coordinate): Coordinate[] => {
    let possibilities: Coordinate[] = [];
    // assume this is the starting node and we should consider everything
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            const y = currPos.y + i;
            const x = currPos.x + j;
            ...
            if (lines[y]) {
                ...
                possibilities.push({ x, y, type: char })
            }
        }
    }
    ...
    possibilities = possibilities.filter(possibility => {
        return (possibility.x === currPos.x && possibility.y !== currPos.y) || possibility.y === currPos.y && possibility.x !== currPos.x
    })

    // filter invalid pipes
    possibilities = possibilities.filter((p, idx) => {
        return isValidPipe(currPos, p, idx)
    })

    // filter visited
    possibilities = possibilities.filter(p => !visited[`${p.x}${p.y}`]);

    return possibilities
}

const visited: { [coordinate: string]: boolean } = {};

const visitNode = (lines: string[], node: Coordinate) => {
    visited[`${node.x}${node.y}`] = true;
    const children = getPossibleNodes(lines, node);
    return children;
}

const traverseLevel = (lines: string[], queue: Coordinate[], steps: number) => {
    if (!queue.length) {
        return steps;
    }
    steps += 1;
    ...
    queue = [];
    queueCopy.forEach(node => {
        const children = visitNode(lines, node)
        queue.push(...children);
    })
    return traverseLevel(lines, queue, steps)
}
...
const parse = (input: string) => {
    ...
    const result = traverseLevel(lines, getPossibleNodes(lines, start), 0);

    return result;
}

const answer = parse(ex4);

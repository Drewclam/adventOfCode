...
let second;
let numbers = [];
const validDigits = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']; // o t s e n
let mem = '';

const test = `cqdfninekrldlfzdnhvcnl8\n`

for (let i = 0; i < test.length; i++) {
    const val = test[i];

    if (val === '\n' || i === test.length - 1) {
        const firstNumStr = validDigits.indexOf(first) === -1 ? first : (validDigits.indexOf(first) + 1).toString();
        const secondNumStr = validDigits.indexOf(second) === -1 ? second : (validDigits.indexOf(second) + 1).toString();
        const num = parseInt(`${firstNumStr}${secondNumStr}`)
        console.log({ num })
        numbers.push(num);
        first = undefined;
        second = undefined;
    ...
    mem += val;
    const digit = validDigits.find(digit => digit.startsWith(mem))

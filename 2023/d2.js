...
const test = `Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`
...
const parseGame = (str) => {
    ...
    const [_, strId] = pieces[0].split(' ');
    const id = parseInt(strId);
    const sets = pieces[1].split(';').map(val => val.trim())
        .map(set => {
            const sets = set.split(',').reduce((set, curr) => {
                const [val, color] = curr.split(' ').filter(str => str.length > 0);
                set[color] = parseInt(val);
                return set;
            }, {});
            return sets;
        })
...
const isValidGame = (game) => {
    return game.sets.every(set => {
        const setDiscrepancy = Object.entries(set).filter(([color, val]) => {
            return val > map[color]
        });
        return !setDiscrepancy.length;
    })
}

const answer1 = sum(input.split('\n').map(parseGame).map(game => {
    if (isValidGame(game)) {
        return game.id
    }
    return 0;
...
const getMinimumCubes = game => {
    ...
    game.sets.forEach(set => {
        Object.entries(set).forEach(([color, val]) => {
            if (!min[color]) {
                min[color] = parseInt(val);
            }
            if (min[color] < parseInt(val)) {
                min[color] = parseInt(val)
            }
            return min;
        })
